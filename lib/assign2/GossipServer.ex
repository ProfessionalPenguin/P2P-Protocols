defmodule Assign2.GossipServer do
  use GenServer

  def start_link(args) do
    message="Started Gossip Server."
    IO.inspect(message)
  ...
  def handle_info(:execute, {nodeCount, topology, taggedNodes, _start_time}) do
    ...
    nodeCount=
      ...
      1..nodeCount|> Enum.map(fn _-> Assign2.GossipNodeSupervisor.add_node(topology) end)|> Assign2.Topologies.create_topology(topology)
  ...
  def handle_cast({:tagged, _pid}, {nodeCount, topology, taggedNodes, start_time}) do
    ...
    if(nodeCount <= 1) do
      ...
      IO.puts("Nodes Tagged: #{taggedNodes} ")
      System.halt(0)
    end
